...
module.exports = async function (req, res) {
    ...
    try {
      ...
      // Note: Keyzy's REST API has no endpoint to update the name/email on an
      // existing license (only "register", which creates a brand-new one), so
      // there is no way to write the claiming customer's email back onto the
      // Keyzy record here. Shopify's metafield (written above) is the real
      // source of truth for who currently has dashboard access to this serial
      // -- Keyzy's own record will only reflect it if edited manually there.
